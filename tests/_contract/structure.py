...
_PERSONAL_PATH = re.compile(r"/(?:mnt/[a-z]/Users|home|Users)/(?!<|\$|\{)([A-Za-z0-9._-]+)/")
...
def script_bearing_skills(skills_dir: Path = SKILLS_DIR) -> list[Path]:
    ...
    return [
        ...
        if skill.is_dir() and (skill / "SKILL.md").is_file() and any((skill / "scripts").rglob("*"))
    ]
...
def frontmatter_problems(skill: Path) -> list[str]:
    ...
    for key, value in entries:
        if value.startswith(("{", "[")):
            problems.append(f"{skill.name}: `{key}` uses JSON flow style; strictyaml rejects it")

    tools = values.get("allowed-tools")
    if tools is not None and ("," in tools or tools.startswith("[")):
        problems.append(f"{skill.name}: `allowed-tools` must be a space-separated string")
...
def stray_test_problems(skill: Path) -> list[str]:
    ...
    return [f"{skill.name}: ships {stray}; tests belong in tests/{skill.name}/" for stray in strays]
...
def bytecode_problems(skill: Path) -> list[str]:
    ...
    return [
        f"{skill.name}: ships bytecode artifact {path.relative_to(skill)}" for path in candidates
    ]
    ...
    for path in _script_paths(skill):
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(function, ast.Name) and function.id in BANNED_BUILTIN_CALLS:
                problems.append(
                    f"{skill.name}: {path.relative_to(skill)}:{node.lineno} calls {function.id}()"
                )
            if (
...
def shadow_module_problems(skill: Path) -> list[str]:
    ...
    """
    return [
        f"{skill.name}: scripts/{path.name} shadows the standard-library module `{path.stem}`"
...
def shell_script_problems(skill: Path) -> list[str]:
    ...
    for path in _script_paths(skill, ".sh"):
        ...
        if syntax.returncode != 0:
            problems.append(f"{skill.name}: {relative} fails `bash -n`: {syntax.stderr.strip()}")
    return problems
