...
def complete_research_workflow():
    ...
    for url in urls:
        source = requests.post(
            f"{BASE_URL}/sources",
            data={"url": url, "notebook_id": notebook_id, "process_async": "true"},
        ).json()
    ...
    for source_id in source_ids:
        while True:
            status = requests.get(f"{BASE_URL}/sources/{source_id}/status").json()
    ...
    session = requests.post(
        ...
        json={"notebook_id": notebook_id, "title": "Resistance Mechanisms"},
    ).json()
    ...
    results = requests.post(
        f"{BASE_URL}/search",
        json={"query": "efflux pump resistance mechanism", "search_type": "vector", "limit": 5},
    ).json()
...
def generate_research_podcast(notebook_id):
    ...
    job = requests.post(
        ...
        json={
            ...
            "speaker_profile_ids": ["speaker_profile:host", "speaker_profile:expert"],
        },
    ).json()
    job_id = job["job_id"]
    ...
    if status["status"] == "completed":
        ...
        audio = requests.get(f"{BASE_URL}/podcasts/episodes/{episode_id}/audio")
...
def create_and_run_transformations():
    ...
    models = requests.get(f"{BASE_URL}/models", params={"model_type": "llm"}).json()
...
def advanced_search(notebook_id, query):
    ...
    sources = requests.get(f"{BASE_URL}/sources", params={"notebook_id": notebook_id}).json()
    ...
    for result in results["results"]:
        print(
            f"  - {result.get('title', 'Untitled')} (similarity: {result.get('similarity', 'N/A')})"
        )
...
def setup_ai_models():
    ...
    discovered = requests.get(f"{BASE_URL}/models/discover/openai").json()
