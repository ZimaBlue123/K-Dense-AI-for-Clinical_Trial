...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="flag a level whose mean recovery deviates more than this %% from 100%%",
    )
    parser.add_argument(
        "--rsd-limit",
        type=float,
    ...
    def precision_block(label: str, groups: dict[str, list[float]], reference: float | None):
        ...
        try:
            ...
            precision_rows.append(
                {
                    "level": label,
                    "component": "repeatability only",
                    "sd": sd,
                    "rsd_pct": rsd,
                    "df": len(vals) - 1,
                    f"ci{pct}_low_sd": lo,
                    f"ci{pct}_high_sd": hi,
                }
            )
        ...
        precision_rows.extend(
            [
                {
                    "level": label,
                    "component": "repeatability (within group)",
                    "sd": comp.sd_repeatability,
                    "rsd_pct": as_rsd(comp.sd_repeatability),
                    "df": comp.df_within,
                    f"ci{pct}_low_sd": sr_lo,
                    f"ci{pct}_high_sd": sr_hi,
                },
                {
                    "level": label,
                    "component": "between-group",
                    "sd": comp.sd_between,
                    "rsd_pct": as_rsd(comp.sd_between),
                    "df": comp.df_between,
                    f"ci{pct}_low_sd": float("nan"),
                    f"ci{pct}_high_sd": float("nan"),
                },
                {
                    "level": label,
                    "component": "intermediate precision (total)",
                    "sd": comp.sd_intermediate,
                    "rsd_pct": as_rsd(comp.sd_intermediate),
                    "df": sat_df,
                    f"ci{pct}_low_sd": si_lo,
                    f"ci{pct}_high_sd": si_hi,
                },
            ]
        )
    ...
    if has_group:
        ...
        for rec in records:
            norm_groups.setdefault(rec["group"], []).append(100.0 * rec["measured"] / rec["level"])
        ...
        for level in levels:
            ...
            precision_rows.append(
                {
                    "level": f"{level:g}",
                    "component": "repeatability (no group column)",
                    "sd": sd,
                    "rsd_pct": rsd,
                    "df": len(vals) - 1,
                    f"ci{pct}_low_sd": lo,
    ...
    if args.design_check:
        ...
        option_a = (
            len(records) >= spec["range_determinations"] and len(levels) >= spec["range_levels"]
        )
