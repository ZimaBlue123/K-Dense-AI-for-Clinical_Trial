...
def render_protocol(
    ...
    if framework == "ich-m10" and modality:
        for row in plan_m10(modality):
            lines.append(f"| {row['item']} | {row['tolerance']} | guideline default | ICH M10 |")
    elif attribute:
...
def main() -> int:
    ...
    parser.add_argument(
        "--modality", choices=sorted(M10_CRITERIA), help="ICH M10 only: chromatographic or lba"
    )
    parser.add_argument(
        "--technique",
        choices=sorted(TECHNIQUE_NOTES),
        help="analytical technique, for Annex 2 notes",
    )
    parser.add_argument(
        "--range-use",
        choices=sorted(Q2R2_REPORTABLE_RANGE),
        help="reportable range example to include",
    )
    parser.add_argument(
        "--protocol", action="store_true", help="emit a validation protocol skeleton in Markdown"
    )
    parser.add_argument("--list-frameworks", action="store_true")
    parser.add_argument(
        "--list-dl-ql", action="store_true", help="list the DL/QL estimation approaches"
    )
    ...
    if args.list_dl_ql:
        rows = [
            {
                "approach": k,
                "detection_limit": v["dl"],
                "quantitation_limit": v["ql"],
                "note": v["note"],
            }
    ...
    if args.protocol:
        print(
            render_protocol(
                args.framework, attribute, args.modality, args.technique, args.range_use
            )
        )
