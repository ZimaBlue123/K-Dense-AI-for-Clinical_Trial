...
def main() -> int:
    ...
    parser.add_argument(
        "--alpha", type=float, default=0.05, help="one-sided alpha for TOST (default 0.05)"
    )
    parser.add_argument(
        "--slope-tolerance",
        type=float,
    ...
    p_naive = 2.0 * (1.0 - t_cdf(abs(t_stat), n - 1)) if math.isfinite(t_stat) else float("nan")
    ...
    summary = [
        ...
        {
            "statistic": "difference 95% CI",
            "value": f"{ba['bias_ci95'][0]:.6g} to {ba['bias_ci95'][1]:.6g}",
        },
        {"statistic": f"SD of differences ({unit})", "value": sd},
        {
            "statistic": "limits of agreement",
            "value": f"{ba['loa_lower']:.6g} to {ba['loa_upper']:.6g}",
        },
        ...
        {
            "statistic": f"{100 * (1 - 2 * args.alpha):.0f}% CI (TOST)",
            "value": f"{tost['ci_1_minus_2alpha'][0]:.6g} to {tost['ci_1_minus_2alpha'][1]:.6g}",
        },
        ...
        {
            "statistic": "Deming slope 95% CI",
            "value": f"{dem['slope_ci95'][0]:.6g} to {dem['slope_ci95'][1]:.6g}",
        },
    ...
    if pb:
        summary += [
            ...
            {
                "statistic": "Passing-Bablok slope 95% CI",
                "value": f"{pb['slope_ci95'][0]:.6g} to {pb['slope_ci95'][1]:.6g}",
            },
