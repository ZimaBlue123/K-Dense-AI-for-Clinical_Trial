...
def check_run(path: str, crit: dict) -> tuple[list[dict], list[str]]:
    ...
    for i, r in enumerate(rows):
        ...
        if kind in ("calibrator", "cal", "standard", "std"):
            ...
            raise InputError(f"row {i + 1}: type must be calibrator or qc, got {r['type']!r}")
...
def check_total_error(path: str, crit: dict) -> tuple[list[dict], list[str]]:
    ...
    for i, r in enumerate(rows):
        ...
        if not ok:
            findings.append(
                f"{label or 'level ' + str(i + 1)}: total error {total:.2f}% exceeds {limit:.0f}%"
            )
    return detail, findings
...
def main() -> int:
    ...
    parser.add_argument(
        "--criteria", action="store_true", help="print the criteria that would be applied and exit"
    )
