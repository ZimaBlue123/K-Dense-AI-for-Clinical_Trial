...
fig = go.Figure()
fig.add_trace(
    go.Scatter(x=[1, 2, 3], y=[5, 10, 15], error_y=dict(type="data", array=[1, 2, 3], visible=True))
)
```
...
# With graph_objects
fig = go.Figure(data=go.Heatmap(z=z_matrix, x=x_labels, y=y_labels, colorscale="RdBu"))
```
...
# Filled contour
fig = go.Figure(data=go.Contour(z=z_matrix, contours=dict(coloring="heatmap", showlabels=True)))
```
...
fig = go.Figure(
    go.Waterfall(
        x=categories, y=values, measure=["relative", "relative", "total", "relative", "total"]
    )
)
```
...
fig = go.Figure(go.Funnel(y=["Stage 1", "Stage 2", "Stage 3"], x=[100, 60, 40]))
```
...
# Country-level
fig = px.choropleth(
    df, locations="iso_alpha", color="value", hover_name="country", color_continuous_scale="Viridis"
)

# US States
fig = px.choropleth(
    df, locations="state_code", locationmode="USA-states", color="value", scope="usa"
)
```
...
fig = px.density_mapbox(
    df, lat="lat", lon="lon", z="value", radius=10, zoom=10, mapbox_style="open-street-map"
)
```
...
```python
fig = go.Figure(
    data=go.Cone(x=x, y=y, z=z, u=u, v=v, w=w, colorscale="Blues", sizemode="absolute", sizeref=0.5)
)
```
...
```python
fig = px.sunburst(df, path=["continent", "country", "city"], values="population", color="value")
```
...
fig = go.Figure(
    data=[
        go.Sankey(
            ...
            link=dict(source=[0, 1, 0, 2, 3], target=[2, 3, 3, 4, 4], value=[8, 4, 2, 8, 4]),
        )
    ]
)
```
...
```python
fig = px.parallel_categories(df, dimensions=["cat1", "cat2", "cat3"], color="value")
```

### Scatter Matrix (SPLOM)

```python
fig = px.scatter_matrix(df, dimensions=["col1", "col2", "col3", "col4"], color="category")
```
...
```python
fig = go.Figure(
    data=[go.Table(header=dict(values=["A", "B", "C"]), cells=dict(values=[col_a, col_b, col_c]))]
)
```
...
fig = px.scatter(
    df, x="log2_fold_change", y="neg_log10_pvalue", color="significant", hover_data=["gene_name"]
)
