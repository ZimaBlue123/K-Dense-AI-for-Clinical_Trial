...
def variational_algorithm(ansatz, cost_function, initial_params):
    ...
    # Optimize
    result = scipy.optimize.minimize(objective, initial_params, method="COBYLA")
...
def my_ansatz(params):
    q = cirq.LineQubit(0)
    return cirq.Circuit(cirq.ry(params[0])(q), cirq.rz(params[1])(q))
...
def noise_comparison_study(circuit, noise_levels):
    ...
    for noise_level in noise_levels:
        ...
        results[noise_level] = {
            ...
            "dominant_state": max(result.histogram(key="result").items(), key=lambda x: x[1]),
        }
