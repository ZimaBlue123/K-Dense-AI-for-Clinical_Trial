...
# Define sweep
sweep = cirq.Product(cirq.Linspace("theta", 0, np.pi, 20), cirq.Linspace("phi", 0, 2 * np.pi, 20))
...
def collect_experiment_data(circuit, sweep, simulator, repetitions=1000):
    ...
    for params, result in zip(sweep, results):
        ...
        data.append({**param_dict, "counts": counts, "total": repetitions})
...
class ExperimentTask:
    ...
    def run(self, qubits, simulator):
        ...
        return {"theta": self.theta, "phi": self.phi, "result": result}
...
def collect_data_parallel(tasks, qubits, simulator, n_workers=4):
    ...
    run_func = functools.partial(run_task_parallel, qubits=qubits, simulator=simulator)
...
def vqe_experiment(hamiltonian, ansatz_func, initial_params):
    ...
    def cost_function(params):
        ...
        energy = hamiltonian.expectation_from_state_vector(
            result.final_state_vector, qubit_map={q: i for i, q in enumerate(circuit.all_qubits())}
        )
        return energy.real
...
def h2_ansatz(params, qubits):
    ...
    theta = params[0]
    return cirq.Circuit(
        cirq.X(qubits[1]), cirq.ry(theta)(qubits[0]), cirq.CNOT(qubits[0], qubits[1])
    )
...
# Run VQE
result = vqe_experiment(hamiltonian, lambda p: h2_ansatz(p, qubits), initial_params=[0.0])
...
def calculate_expectation_value(circuit, observable, simulator):
    ...
    # Remove measurements
    circuit_no_measure = cirq.Circuit(m for m in circuit if not isinstance(m, cirq.MeasurementGate))
    ...
    expectation = observable.expectation_from_state_vector(
        state_vector, qubit_map={q: i for i, q in enumerate(circuit.all_qubits())}
    )
...
def process_fidelity(result1, result2):
    ...
    fidelity = sum(np.sqrt(probs1.get(s, 0) * probs2.get(s, 0)) for s in all_states) ** 2
...
class VQEExperiment(QuantumExperiment):
    ...
    def cost_function(self, params):
        ...
        energy = self.hamiltonian.expectation_from_state_vector(
            result.final_state_vector, qubit_map={q: i for i, q in enumerate(self.qubits)}
        )
    ...
    def run(self, initial_params):
        result = scipy.optimize.minimize(
            self.cost_function, initial_params, method="COBYLA", options={"maxiter": 100}
        )
        return result
