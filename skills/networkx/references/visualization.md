...
# Circle nodes
nx.draw_networkx_nodes(G, pos, nodelist=[0, 1, 2], node_shape="o", node_color="red")

# Square nodes
nx.draw_networkx_nodes(G, pos, nodelist=[3, 4, 5], node_shape="s", node_color="blue")
...
edge_colors = ["red" if G[u][v].get("weight", 1) > 0.5 else "blue" for u, v in G.edges()]
...
nx.draw(
    ...
    pos=pos,
    with_labels=True,
    node_color=["lightblue" if B.nodes[n]["bipartite"] == 0 else "lightgreen" for n in B.nodes()],
)
plt.show()
```
