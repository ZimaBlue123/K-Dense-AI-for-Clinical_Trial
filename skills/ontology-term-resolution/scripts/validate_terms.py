...
def read_pairs(args: argparse.Namespace) -> list[tuple[str, str | None]]:
    ...
    if any(cell in ID_HEADERS for cell in header):
        ...
        label_index = next((i for i, cell in enumerate(header) if cell in LABEL_HEADERS), None)
        rows = rows[1:]
...
def check_term(
    ...
    *,
    ...
    if expect_ontologies and result["ontology"] not in expect_ontologies:
        ...
        result["detail"] = f"defined by {result['ontology']!r}, expected one of " + ",".join(
            sorted(expect_ontologies)
        )
        return result
    ...
    if expected_label is not None:
        ...
        if normalize_label(result["actual_label"]) != wanted:
            if any(normalize_label(s) == wanted for s in synonyms_of(term)):
                ...
                result["detail"] = f"claimed {expected_label!r}, actual {result['actual_label']!r}"
                return result
    ...
    if not term.get("is_defining_ontology") and result["ontology"] != home:
        warnings.append(
            (
                "imported_only",
                f"{home} does not define this term; found only as a copy in {result['ontology']!r}",
            )
        )

    if term.get("type") not in (None, "class"):
        warnings.append(("not_a_class", f"term type is {term.get('type')!r}, not a class"))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--branch", help="require every term to be a descendant of this CURIE")
    parser.add_argument(
        "--expect-ontology",
