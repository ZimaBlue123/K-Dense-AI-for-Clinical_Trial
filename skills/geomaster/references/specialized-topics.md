...
def fit_variogram_model(lags, gammas, model="spherical"):
    ...
    def spherical(h, nugget, sill, range_):
        ...
        h = np.asarray(h)
        gamma = np.where(
            h < range_, nugget + sill * (1.5 * h / range_ - 0.5 * (h / range_) ** 3), nugget + sill
        )
        return gamma
...
def kriging_cross_validation(x, y, z, n_folds=5):
    ...
    for train_idx, test_idx in kf.split(z):
        # Train
        OK = OrdinaryKriging(
            x[train_idx], y[train_idx], z[train_idx], variogram_model="spherical", verbose=False
        )

        # Predict at test locations
        predictions, _ = OK.execute("points", x[test_idx], y[test_idx])
...
def traveling_salesman(G, start_node):
    ...
    while unvisited:
        ...
        nearest = min(unvisited, key=lambda n: G[current][n].get("weight", 1))
