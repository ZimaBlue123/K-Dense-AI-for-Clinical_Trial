...
# 20. Choropleth
folium.Choropleth(gdf, data=stats, columns=["id", "value"], key_on="feature.properties.id").add_to(
    m
)
...
pdk.Deck(
    layers=[pdk.Layer("ScatterplotLayer", data=df)], map_style="mapbox://styles/mapbox/dark-v9"
)
...
resampled = dst.read(out_shape=(src.height * 2, src.width * 2), resampling=Resampling.bilinear)

# 78. Create regular grid
from shapely.geometry import box

grid = [
    box(xmin, ymin, xmin + dx, ymin + dy)
