...
def full_factorial(factors, randomize=True, seed=0):
    ...
    data = {n: [levels[j][coded[i, j]] for i in range(len(coded))] for j, n in enumerate(names)}
...
def fractional_factorial(factors, generator, randomize=True, seed=0):
    ...
    if coded.shape[1] != len(factors):
        raise ValueError(
            f"generator defines {coded.shape[1]} factors but {len(factors)} were named"
        )
    return _randomize(_decode_two_level(coded, factors), randomize, seed)
...
def central_composite(
    factors, center=(0, 1), alpha="orthogonal", face="circumscribed", randomize=True, seed=0
):
...
def latin_hypercube(factors, n_samples, criterion="maximin", seed=0, randomize=False):
...
if __name__ == "__main__":
    ...
    print(
        central_composite({"temp": (20, 60), "conc": (1, 10)}, seed=1)
        .round(2)
