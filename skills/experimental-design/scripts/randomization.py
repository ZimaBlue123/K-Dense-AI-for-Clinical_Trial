...
def block_randomization(n, arms=("treatment", "control"), block_size=None, ratio=None, seed=0):
    ...
    if block_size % unit != 0:
        raise ValueError(f"block_size ({block_size}) must be a multiple of sum(ratio)={unit}")
...
def stratified_block_randomization(
    strata, arms=("treatment", "control"), block_size=None, ratio=None, seed=0
):
    ...
    for i, (label, count) in enumerate(items):
        df = block_randomization(
            count, arms=arms, block_size=block_size, ratio=ratio, seed=seed + 1 + i
        )
...
def cluster_randomization(
    clusters, arms=("treatment", "control"), ratio=None, block_size=None, seed=0
):
    ...
    df = block_randomization(
        len(clusters), arms=arms, ratio=ratio, block_size=block_size, seed=seed
    )
