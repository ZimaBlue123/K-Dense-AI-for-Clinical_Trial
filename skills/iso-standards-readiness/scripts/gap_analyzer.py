...
def analyze(
    ...
    *,
    ...
    expected = (
        {item for item in expected_raw if isinstance(item, str) and item in profile.process_domains}
    ...
    by_domain: dict[str, list[dict[str, Any]]] = {domain: [] for domain in profile.process_domains}
    for item in entries:
    ...
    for domain in profile.process_domains:
        ...
        if domain not in expected:
            ...
            explanation = (
                "At least one submitted entry is draft, unapproved, unsourced, or missing evidence."
            )
        statuses[status] += 1
