    ...
    def doi_to_bibtex(self, doi: str) -> str | None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error: Failed to retrieve BibTeX for {doi} (status {response.status_code})",
                    file=sys.stderr,
                )
                return None
...
def main():
    ...
    if args.output:
        try:
            with open(args.output, "w", encoding="utf-8") as f:
                f.write(output)
            print(
                f"Successfully wrote {len(bibtex_entries)} entries to {args.output}",
                file=sys.stderr,
            )
        except Exception as e:
    ...
    if len(dois) > 1:
        ...
        print(
            f"\nConverted {len(bibtex_entries)}/{len(dois)} DOIs ({success_rate:.1f}%)",
            file=sys.stderr,
        )
