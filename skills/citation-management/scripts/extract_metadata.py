    ...
    def extract_from_doi(self, doi: str) -> dict | None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error: CrossRef API returned status {response.status_code} for DOI: {doi}",
                    file=sys.stderr,
                )
                return None
    ...
    def extract_from_pmid(self, pmid: str) -> dict | None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error: PubMed API returned status {response.status_code} for PMID: {pmid}",
                    file=sys.stderr,
                )
                return None
    ...
    def extract_from_arxiv(self, arxiv_id: str) -> dict | None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if entry is None:
                    print(
                        f"Error: No entry found for arXiv ID: {arxiv_id}",
                        file=sys.stderr,
                    )
                    return None
                ...
                print(
                    f"Error: arXiv API returned status {response.status_code} for ID: {arxiv_id}",
                    file=sys.stderr,
                )
                return None
...
def main():
    ...
    if args.output:
        with open(args.output, "w", encoding="utf-8") as f:
            f.write(output)
        print(
            f"\nSuccessfully wrote {len(bibtex_entries)} entries to {args.output}",
            file=sys.stderr,
        )
    else:
        print(output)
