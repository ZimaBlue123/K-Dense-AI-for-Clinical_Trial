...
try:
    ...
    print(
        "Warning: scholarly library not installed. Install with: pip install scholarly",
        file=sys.stderr,
    )
...
def main():
    ...
    parser.add_argument(
        "--use-proxy", action="store_true", help="Use free proxy to avoid rate limiting"
    )
    ...
    if not SCHOLARLY_AVAILABLE:
        ...
        print(
            "\nAlternatively, use PubMed search for biomedical literature:",
            file=sys.stderr,
        )
    ...
    if args.format == "json":
        output = json.dumps(
            {"query": args.query, "count": len(results), "results": results}, indent=2
        )
    else:  # bibtex
