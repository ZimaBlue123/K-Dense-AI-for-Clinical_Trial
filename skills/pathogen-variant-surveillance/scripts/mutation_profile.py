...
def profile(
    base_url: str, filters: dict, *, amino_acid: bool, min_proportion: float, gene: str | None
) -> dict[str, dict]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--instance", default="sars-cov-2", help="registry name (default: sars-cov-2)"
    )
    ...
    parser.add_argument(
        "--nucleotide", action="store_true", help="nucleotide instead of amino-acid mutations"
    )
    parser.add_argument(
        "--min-proportion",
        type=float,
    ...
    parser.add_argument(
        "--where", action="append", default=[], metavar="KEY=VALUE", help="extra filter, repeatable"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.versus:
            ...
            rows.sort(
                key=lambda r: (
                    {"gained": 0, "lost": 1, "shared": 2}[r["verdict"]],
                    str(r["gene"]),
                    int(r["position"] or 0),
                )
            )
