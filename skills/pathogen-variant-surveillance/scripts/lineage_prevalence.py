...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--weeks", type=int, default=26, help="window length in weeks (default: 26)"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for name in targets:
            ...
            counts, _ = weekly_counts(base_url, {**window, lineage_field: value}, date_field, weeks)
    ...
    if args.format != "json":
        print(
            ...
            + (
                f"\n# {undated} matching sequences carry no usable collection date"
                if undated
                else ""
            ),
            file=sys.stderr,
