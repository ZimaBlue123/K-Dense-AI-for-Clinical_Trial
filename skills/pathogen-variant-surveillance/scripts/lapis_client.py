...
def aggregated(base_url: str, filters: dict[str, Any], fields: Sequence[str] = ()) -> list[dict]:
...
def looks_like_lineage(name: str) -> bool:
    ...
    if any(token in low for token in ("_seg", "qc", "score", "coverage", "version")):
        return False
    return any(token in low for token in ("clade", "lineage", "genotype", "serotype", "subtype"))
