        ...
        rows.append(
            {
                ...
                "responder_fraction": float(values.mean())
                if set(np.unique(values).tolist()) <= {0.0, 1.0}
                else float("nan"),
            }
        )
    return rows
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--cmax", type=float, help="exposure at which to evaluate the C-QTc prediction"
    )
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    exposure = np.asarray(
        [parse_float(r[args.exposure_column], "exposure") for r in rows], dtype=float
    )
    response = np.asarray(
        [parse_float(r[args.response_column], "response") for r in rows], dtype=float
    )
    ...
    if args.emax:
        ...
        report.note(
            "a linear-looking E-R relationship is the low-concentration limb of an Emax curve; the two are not distinguishable without data near the plateau"
        )

    elif args.logistic:
        fit = fit_logistic(exposure, response)
        report.table(
            "logistic exposure-response", [{"parameter": k, "value": v} for k, v in fit.items()]
        )
        ...
        if math.isfinite(fit["slope_p"]) and fit["slope_p"] > 0.05:
            report.note(
                f"the exposure slope is not statistically distinguishable from zero (p = {fit['slope_p']:.3f})"
            )
