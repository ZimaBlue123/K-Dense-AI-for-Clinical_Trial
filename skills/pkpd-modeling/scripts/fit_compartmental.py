...
def predictor(spec: FitSpec) -> Callable[[np.ndarray, np.ndarray], np.ndarray]:
    ...
    def predict(times: np.ndarray, theta: np.ndarray) -> np.ndarray:
        ...
        if spec.absorption == "transit":
            ...
            for dose in regimen:
                total += conc_transit(
                    np.maximum(times - dose.time - tlag, 0.0), dose.amount, mtt, ntr, disp
                )
            return total
        ka = extra[0]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--interval", type=float, help="dosing interval for a multiple-dose profile"
    )
    ...
    parser.add_argument(
        "--max-rse", type=float, default=50.0, help="flag %%RSE above this (default: 50)"
    )
    parser.add_argument(
        "--max-corr", type=float, default=0.95, help="flag |correlation| above this (default: 0.95)"
    )
    parser.add_argument(
        "--predictions", action="store_true", help="also emit the observed/predicted table"
    )
...
def _fit_and_report(spec: FitSpec, args, subject: str, time, conc, report: Report) -> FitResult:
        ...
        rows.append(
            {
                ...
                "ci95_low": result.theta[i] * math.exp(-crit * se)
                if math.isfinite(se)
                else float("nan"),
                "ci95_high": result.theta[i] * math.exp(crit * se)
                if math.isfinite(se)
                else float("nan"),
            }
        )
    ...
    report.table(
        f"{label}: secondary parameters",
        [{"parameter": k, "value": v} for k, v in secondary.items()],
    )
    ...
    report.table(
        ...
        [
            ...
            {
                "statistic": "runs-test p (residual signs)",
                "value": runs_test_p(result.weighted_residuals),
            },
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    report.note(
        "parameters estimated on the log scale; confidence intervals are therefore asymmetric"
    )
    if args.route == "oral":
        report.note(
    ...
    for subject, subject_rows in subjects.items():
        time = np.asarray(
            [parse_float(r[args.time_column], "time") for r in subject_rows], dtype=float
        )
        conc = np.asarray(
            [parse_float(r[args.conc_column], "conc") for r in subject_rows], dtype=float
        )
        ...
        if np.sum(keep) < len(conc):
            report.note(
                f"subject {subject or '1'}: dropped {int(np.sum(~keep))} non-positive concentration(s)"
            )
            ...
            for index, (name, fit) in enumerate(ordered):
                ...
                if index > 0:
                    ...
                    if d_p > 0 and fit.wssr > 0 and fit.n - fit.p > 0:
                        ...
                        entry["f_p_value"] = (
                            float(1.0 - f_dist.cdf(f_stat, d_p, fit.n - fit.p))
                            if f_stat > 0
                            else 1.0
                        )
