...
def disposition(
    cl: float, v1: float, q: Sequence[float] = (), vp: Sequence[float] = ()
) -> Disposition:
    ...
    if len(q) != len(vp):
        raise ValueError(
            f"got {len(q)} intercompartmental clearances but {len(vp)} peripheral volumes"
        )
    if cl <= 0 or v1 <= 0:
...
def conc_oral(
    t: np.ndarray, dose: float, ka: float, d: Disposition, f: float = 1.0, tlag: float = 0.0
) -> np.ndarray:
...
def conc_transit(
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        log_rate = (
            math.log(f * dose)
            + math.log(ktr)
            + n * np.log(np.maximum(grid, 1e-300) * ktr)
            - ktr * grid
            - math.lgamma(n + 1.0)
        )
...
def steady_state_metrics(
    d: Disposition, dose: float, interval: float, f: float = 1.0
) -> dict[str, float]:
...
def _integrate_with_doses(
    ...
    infusions = [
        (e.time, e.time + e.duration, e.amount / e.duration) for e in events if e.duration > 0
    ]
    ...
    breakpoints = sorted(
        {
            ...
            *(e.time + e.duration for e in events if e.duration > 0),
            float(np.max(times)),
        }
    )
    ...
    for index, start in enumerate(breakpoints):
        ...
        if not solution.success:  # pragma: no cover - solver failure path
            raise RuntimeError(
                f"integration failed between t={start} and t={stop}: {solution.message}"
            )
        if np.any(window):
...
def emax(
    conc: np.ndarray, e0: float, emax_value: float, ec50: float, hill: float = 1.0
) -> np.ndarray:
...
def imax(
    conc: np.ndarray, e0: float, imax_value: float, ic50: float, hill: float = 1.0
) -> np.ndarray:
...
def indirect_response(
    ...
    *how* the drug perturbs turnover, not in where the response starts. This is
    ...
    solution = solve_ivp(
        ...
        atol=1e-10,
        dense_output=True,
        max_step=max(span[1] / 200.0, 1e-6),
    )
