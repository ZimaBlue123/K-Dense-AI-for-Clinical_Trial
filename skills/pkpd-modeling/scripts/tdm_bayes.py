...
def _level(text: str) -> tuple[float, float]:
    if "@" not in text:
        raise argparse.ArgumentTypeError(
            f"--level must look like 18.2@11.5 (conc@time), got {text!r}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--custom", action="store_true", help="supply population parameters directly"
    )
    ...
    parser.add_argument(
        "--infusion", type=float, default=1.0, help="infusion duration, h (default: 1)"
    )
    parser.add_argument(
        "--doses-given",
        type=int,
        default=20,
        help="doses administered before the levels (default: 20)",
    )
    parser.add_argument(
        "--level", type=_level, action="append", required=True, help="conc@time-after-last-dose"
    )
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    current = exposure_metrics(
        result["cl_individual"], result["v_individual"], args.dose, args.interval, args.infusion
    )
    report.table(
        "predicted exposure on the current regimen",
        [{"metric": k, "value": v} for k, v in current.items()],
    )
    ...
    if not result["converged"]:
        report.finding(
            "the MAP optimiser did not converge; treat the individual estimates as unreliable"
        )
