...
def crossover_2x2(records: list[dict], design: str = "2x2") -> AverageBE:
    ...
    for row in records:
        by_subject.setdefault(row["subject"], {}).setdefault(row["treatment"], []).append(
            row["logvalue"]
        )
        if row.get("sequence"):
    ...
    if len(differences) < 2:
        ...
        pooled_num = sum(
            float(np.var(v, ddof=1)) * (len(v) - 1) for v in differences.values() if len(v) > 1
        )
...
def reference_variability(records: list[dict]) -> ReferenceVariability:
    ...
    for subject in {r["subject"] for r in records}:
        values = [
            r["logvalue"] for r in records if r["subject"] == subject and r["treatment"] == "R"
        ]
        if len(values) < 2:
            continue
...
def rsabe_bound(
    estimate: float, se: float, df_point: int, rv: ReferenceVariability
) -> dict[str, float]:
...
def sample_size(
    ...
    for n in range(4, 5002, step):
        ...
        if power >= target:
            return n, power
    raise InputError(
        "no sample size below 5000 reaches the target power; the GMR is too far from 1"
    )
...
def load_records(args: argparse.Namespace) -> list[dict]:
    ...
    require_columns(
        rows, [args.subject_column, args.treatment_column, args.value_column], str(args.input)
    )
    records = []
    for index, row in enumerate(rows, start=1):
        ...
        if treatment in {"T", "TEST"}:
            ...
            raise InputError(
                f"row {index}: treatment must be T or R, got {row[args.treatment_column]!r}"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--metric", default="", help="label for the metric being tested, used in output only"
    )
    ...
    parser.add_argument(
        "--limits",
        default="0.80,1.25",
        help="acceptance limits for average BE (default: 0.80,1.25)",
    )
    parser.add_argument(
        "--nti", action="store_true", help="narrow therapeutic index: apply 90.00-111.11%% limits"
    )
    parser.add_argument(
        "--power",
        action="store_true",
        help="prospective power / sample size instead of an analysis",
    )
    parser.add_argument(
        "--cv", type=float, help="assumed within-subject CV (as a fraction) for --power"
    )
    parser.add_argument(
        "--gmr", type=float, default=0.95, help="assumed true GMR for --power (default: 0.95)"
    )
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    result = (
        parallel_design(records)
        if args.design == "parallel"
        else crossover_2x2(records, args.design)
    )
    ...
    if args.scaling != "none":
        ...
        if args.scaling in {"abel", "both"}:
            ...
            abel_pass = (
                low <= result.ci_low
                and result.ci_high <= high
                and limits[0] <= result.gmr <= limits[1]
            )
            rows.append(
                {
                    "criterion": "EMA ABEL",
