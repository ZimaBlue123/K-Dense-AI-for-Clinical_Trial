...
def maturation_fraction(
    pma_weeks: float, tm50: float = DEFAULT_TM50_WEEKS, hill: float = DEFAULT_MATURATION_HILL
) -> float:
...
def fit_exponent(weights: np.ndarray, values: np.ndarray) -> dict[str, float]:
    ...
    se_slope = (
        math.sqrt(ss_res / (n - 2) / float(np.sum((x - x.mean()) ** 2))) if n > 2 else float("nan")
    )
    return {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    mode.add_argument(
        "--scale", action="store_true", help="scale parameters between two body weights"
    )
    mode.add_argument(
        "--exponent", action="store_true", help="estimate the allometric exponent from species data"
    )
    mode.add_argument(
        "--fih", action="store_true", help="NOAEL-based human equivalent dose and MRSD"
    )
    mode.add_argument(
        "--mabel", action="store_true", help="minimum anticipated biological effect level"
    )
    ...
    parser.add_argument(
        "--weight-from", type=float, default=70.0, help="reference weight, kg (default: 70)"
    )
    ...
    parser.add_argument(
        "--pma-weeks", type=float, help="post-menstrual age; adds a maturation factor"
    )
    ...
    parser.add_argument(
        "--human-weight", type=float, default=60.0, help="kg, for converting MRSD to a total dose"
    )

    parser.add_argument(
        "--ec50", type=float, help="in vitro potency (same units as the target concentration)"
    )
    parser.add_argument(
        "--target-occupancy",
        type=float,
        default=0.10,
        help="fraction of target engagement to aim for",
    )
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    if args.scale:
        ...
        rows = []
        maturation = (
            maturation_fraction(args.pma_weeks, args.tm50, args.maturation_hill)
            if args.pma_weeks
            else 1.0
        )
        ...
        report.note(
            "volume is not matured: maturation describes eliminating capacity, not distribution space"
        )
        if args.pma_weeks:
        ...
        if not (fit["ci95_low"] <= 0.75 <= fit["ci95_high"]):
            report.note(
                "the 95% interval excludes 0.75; a fixed-exponent model would be misspecified here"
            )
        report.note(
