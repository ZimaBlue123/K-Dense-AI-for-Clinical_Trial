...
def inlet_concentration(
    imax: float, fu: float, dose: float, fa: float, fg: float, ka: float, blood_ratio: float = 1.0
) -> float:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--basic",
        action="store_true",
        help="run the basic models and report against their cut-offs",
    )
    parser.add_argument("--msm", action="store_true", help="run the mechanistic static model")

    parser.add_argument(
        "--imax", type=float, help="maximum total systemic plasma concentration of the perpetrator"
    )
    parser.add_argument(
        "--fu", type=float, default=1.0, help="unbound fraction in plasma (default: 1.0)"
    )
    parser.add_argument(
        "--dose", type=float, help="perpetrator molar dose, for the gut concentration"
    )
    parser.add_argument(
        "--ka",
        type=float,
        default=0.1,
        help="perpetrator absorption rate constant, per min (default: 0.1)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="perpetrator gut availability (default: 1.0)",
    )
    parser.add_argument(
        "--blood-ratio", type=float, default=1.0, help="blood-to-plasma ratio (default: 1.0)"
    )
    ...
    parser.add_argument(
        "--kdeg",
        type=float,
        default=DEFAULT_KDEG_HEPATIC,
        help="hepatic enzyme degradation rate, per min",
    )
    parser.add_argument(
        "--ind-emax", type=float, default=0.0, help="maximum fold induction minus one"
    )
    parser.add_argument("--ind-ec50", type=float, help="induction EC50")
    parser.add_argument(
        "--ind-scaling", type=float, default=1.0, help="induction scaling factor d (default: 1.0)"
    )

    parser.add_argument(
        "--fm", type=float, help="fraction of victim clearance via the affected enzyme"
    )
    parser.add_argument(
        "--fg",
        type=float,
        default=1.0,
        help="victim fraction escaping gut metabolism (default: 1.0)",
    )
    parser.add_argument(
        "--transporter",
        choices=("hepatic-uptake", "intestinal", "renal"),
        help="also run the transporter basic model",
    )
    ...
    if args.basic:
        ...
        if args.transporter:
            ...
            if args.transporter == "hepatic-uptake":
                ...
                cutoff, triggers = (
                    CUTOFF_TRANSPORTER_HEPATIC_UPTAKE,
                    value >= CUTOFF_TRANSPORTER_HEPATIC_UPTAKE,
                )
                ...
                cutoff, triggers = (
                    CUTOFF_TRANSPORTER_INTESTINAL,
                    value >= CUTOFF_TRANSPORTER_INTESTINAL,
                )
        ...
        if not rows:
            raise InputError(
                "--basic needs at least one of --ki, --tdi, or --ind-ec50 with --ind-emax"
            )
        ...
        report.table(
            "mechanistic static model", [{"term": k, "value": v} for k, v in result.items()]
        )
        ...
        if gut is None:
            report.note(
                "no --dose given, so the intestinal component was evaluated at zero inhibitor concentration"
            )
        if args.fm > 0.95:
            report.finding(
