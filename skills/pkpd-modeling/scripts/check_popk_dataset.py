        ...
        self.items.append(
            {
                ...
                "detail": detail,
                "n_records": len(rows),
                "example_rows": ", ".join(str(r) for r in listed)
                + ("..." if len(rows) > 8 else ""),
            }
        )
...
def check_dataset(rows: list[dict[str, str]], args: argparse.Namespace) -> tuple[Findings, dict]:
    ...
    for required, name in (
        ...
        (dv_col, args.dv_column),
    ):
        if required is None:
            found.add(
                "error",
                "required column",
                f"no '{name}' column; NONMEM cannot build a dataset without it",
            )
    if amt_col is None:
        found.add(
            "error", "required column", "no AMT column: without it no dose is ever administered"
        )
    if evid_col is None:
        found.add(
            "warning",
    ...
    if long_names:
        found.add(
            "note", "column names", f"unusually long column name(s): {', '.join(long_names[:4])}"
        )
    ...
    for index, row in enumerate(rows, start=2):  # header is line 1
        ...
        if time is None:
            found.add(
                "error", "TIME not numeric", f"row {index}: TIME is {row.get(time_col)!r}", [index]
            )
    ...
    if neg_time:
        found.add(
            "error", "negative TIME", "TIME before zero without --allow-negative-time", neg_time
        )
    if neg_conc:
        found.add(
            "warning", "negative DV", "negative concentrations in observation records", neg_conc
        )
    if bad_evid:
    ...
    if dose_without_amt:
        found.add(
            "error",
            "dose without AMT",
            "EVID marks a dose but AMT is missing or not positive",
            dose_without_amt,
        )
    if obs_with_amt:
        found.add(
    ...
    if obs_missing_dv:
        found.add(
            "error",
            "missing DV with MDV=0",
            "observation record has no DV but is not flagged MDV=1",
            obs_missing_dv,
        )
    if mdv_conflict:
        found.add(
            "warning",
            "MDV=1 with a DV present",
            "the observation will be ignored by the estimation",
            mdv_conflict,
        )
    if rate_issues:
        found.add(
            "error",
            "invalid RATE",
            "RATE must be >0, or -1 (modelled duration), or -2 (modelled rate)",
            rate_issues,
        )
    if ss_missing_ii:
        found.add(
            "error", "SS without II", "a steady-state record needs a positive II", ss_missing_ii
        )
    if addl_missing_ii:
        found.add(
            "error",
            "ADDL without II",
            "ADDL repeats a dose every II; with no II the extra doses never happen",
            addl_missing_ii,
        )
    if zero_ii:
        found.add(
            "error",
            "ADDL with II<=0",
            "II must be positive for ADDL to place additional doses",
            zero_ii,
        )
    ...
    for subject, records in subjects.items():
        if subject == "":
            found.add(
                "error", "blank ID", "records with an empty ID column", [r[0] for r in records]
            )
            continue
        times = []
        n_obs = n_dose = 0
        ...
        if records:
            ...
            is_dose_first = (
                (first_evid in {"1", "4"}) if evid_col else bool(first_amt and first_amt > 0)
            )
    ...
    if no_dose:
        found.add(
            ...
            f"{len(no_dose)} subject(s) have observations but no dose: {', '.join(no_dose[:6])}",
        )
    if unsorted:
        found.add(
            "error",
            "TIME not sorted",
            f"{len(unsorted)} subject(s) have out-of-order TIME: {', '.join(unsorted[:6])}",
        )
    if duplicated:
        found.add(
            "warning",
    ...
    for name in args.covariates:
        ...
        if cov_col is None:
            found.add(
                "error",
                "covariate absent",
                f"--covariates names '{name}' but there is no such column",
            )
            continue
        ...
        if non_numeric:
            found.add(
                "error",
                f"covariate {name} non-numeric",
                "values NM-TRAN cannot read as numbers",
                non_numeric,
            )
    ...
    summary = {
        ...
        "median_observations_per_subject": sorted(obs_counts)[len(obs_counts) // 2]
        if obs_counts
        else 0,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--covariates", default="", help="comma-separated covariate columns to check"
    )
    parser.add_argument(
        "--time-varying", default="", help="covariates that are expected to change within a subject"
    )
