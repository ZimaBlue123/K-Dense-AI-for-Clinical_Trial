...
def cumulative_auc(
    time: np.ndarray, conc: np.ndarray, method: str
...
def interpolate(time: np.ndarray, conc: np.ndarray, target: float, method: str) -> float:
    ...
    use_log = (
        method in {"linup-logdown", "log"} and c0 > 0 and c1 > 0 and (c1 < c0 or method == "log")
    )
    if not use_log:
...
def estimate_lambda_z(
    ...
    if manual is not None:
        ...
        if len(idx) < 2:
            return LambdaZ(
                reason=f"manual window {lo}-{hi} contains {len(idx)} quantifiable points"
            )
    ...
    if best.lam is not None and best.lam <= 0:
        return LambdaZ(
            reason=f"terminal slope is not negative (lambda_z = {best.lam:.4g})",
...
def analyse(profile: Profile, args: argparse.Namespace) -> tuple[dict, LambdaZ, list[str]]:
    ...
    if lz.lam:
        ...
        row["pct_aumc_extrap"] = (
            100.0 * (aumc_inf - aumc_last) / aumc_inf if aumc_inf > 0 else float("nan")
        )
    ...
    if profile.tau:
        ...
        cmin = (
            float(np.min(conc[(time >= 0) & (time <= tau) & quantifiable]))
            if np.any(quantifiable)
            else float("nan")
        )
        cavg = auc_tau / tau
        row["cavg_ss"] = cavg
    ...
    if peak == len(time) - 1:
        findings.append(
            f"{prefix}: Cmax is the last sample; the peak and the terminal phase are not characterised"
        )
    if peak == 0 and args.route == "extravascular":
        findings.append(
            f"{prefix}: Cmax is the first sample; the true peak may precede the first draw"
        )
...
def summarise(rows: Sequence[dict], keys: Sequence[str]) -> list[dict]:
    ...
    for key in keys:
        ...
        if np.all(arr > 0):
            ...
            entry["geo_cv_pct"] = (
                float(100.0 * math.sqrt(math.exp(float(logs.var(ddof=1))) - 1.0))
                if len(arr) > 1
                else float("nan")
            )
        out.append(entry)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-i", "--input", required=True, help="concentration-time table (CSV/TSV, or - for stdin)"
    )
    parser.add_argument("--dose", type=float, help="dose given to every subject")
    parser.add_argument(
        "--dose-column", default="dose", help="per-subject dose column (default: dose)"
    )
    ...
    parser.add_argument(
        "--tau", type=float, help="dosing interval; requests steady-state parameters"
    )
    parser.add_argument(
        "--auc-method",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="minimum points in the terminal fit (default: 3)",
    )
    parser.add_argument(
        "--lambda-z-window", type=_window_spec, help="force the terminal window, e.g. 8-48"
    )
    parser.add_argument(
        "--partial-auc",
        type=_partial_spec,
        action="append",
        default=[],
        help="e.g. --partial-auc 0-24",
    )
    parser.add_argument(
        "--min-r2-adj",
        type=float,
        default=0.80,
        help="flag terminal fits below this (default: 0.80)",
    )
    parser.add_argument(
        "--max-extrap",
        type=float,
        default=20.0,
        help="flag %%AUC extrapolated above this (default: 20)",
    )
    parser.add_argument(
        "--min-span",
        type=float,
        default=2.0,
        help="flag lambda_z spans below this many half-lives (default: 2)",
    )
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.no_summary and len(rows) > 1:
        ...
        keys += [
            k
            for k in rows[0]
            if k.startswith("auc_") and k not in keys and k not in {"auc_last", "auc_tau"}
        ]
