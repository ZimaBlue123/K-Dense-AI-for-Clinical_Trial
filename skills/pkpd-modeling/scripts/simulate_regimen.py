...
def _regimen_spec(text: str) -> tuple[float, float]:
    if "@" not in text:
        raise argparse.ArgumentTypeError(
            f"regimen must look like 500@12 (dose@interval), got {text!r}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="intercompartmental clearance (repeatable)",
    )
    parser.add_argument(
        "--v2",
        type=float,
        action="append",
        default=[],
        help="peripheral volume (repeatable, pairs with --q)",
    )
    ...
    parser.add_argument(
        "--tinf", type=float, default=0.0, help="infusion duration for --route iv-infusion"
    )
    ...
    parser.add_argument(
        "--steady-state", action="store_true", help="report closed-form steady-state metrics"
    )
    parser.add_argument("--compare", help="comma-separated dose@interval regimens to compare")

    parser.add_argument(
        "--nonlinear", action="store_true", help="Michaelis-Menten elimination instead of linear"
    )
    parser.add_argument(
        "--vmax", type=float, help="maximum elimination rate (amount/time) for --nonlinear"
    )
    parser.add_argument(
        "--km", type=float, help="Michaelis constant (concentration) for --nonlinear"
    )

    parser.add_argument(
        "--simulate", type=int, help="number of virtual subjects for a Monte Carlo simulation"
    )
    parser.add_argument(
        "--omega-cl", type=float, default=0.0, help="between-subject CV of clearance"
    )
    parser.add_argument("--omega-v", type=float, default=0.0, help="between-subject CV of volume")
    parser.add_argument("--seed", type=int, default=20260727)
    parser.add_argument(
        "--target-trough", type=float, help="report the fraction of subjects above this trough"
    )
    parser.add_argument(
        "--target-peak-below",
        type=float,
        help="report the fraction of subjects with a peak below this",
    )
    parser.add_argument(
        "--target-auc",
        type=float,
        help="report the fraction of subjects above this AUC over the interval",
    )

    parser.add_argument(
        "--profile", action="store_true", help="also emit the concentration-time profile"
    )
...
def run(argv: Sequence[str] | None = None) -> int:
        ...
        report.table(
            ...
            [
                ...
                {"interval": f"last (#{args.n_doses})", **last},
                {
                    ...
                    "auc_tau": last["auc_tau"] / first["auc_tau"]
                    if first["auc_tau"]
                    else float("nan"),
    ...
    if args.steady_state and not args.nonlinear:
        metrics = steady_state_metrics(
            disposition(args.cl, args.v, args.q, args.v2), args.dose, interval, f=args.f
        )
        report.table(
            "closed-form steady state", [{"metric": k, "value": v} for k, v in metrics.items()]
        )
    ...
    if args.simulate:
        ...
        report.table(
            ...
            [
                percentiles(peaks, "peak"),
                percentiles(troughs, "trough"),
                percentiles(aucs, "auc_over_interval"),
            ],
        )
        ...
        if args.target_trough is not None:
            ...
            attainment.append(
                {"target": f"trough >= {args.target_trough:g}", "fraction_attaining": fraction}
            )
        if args.target_peak_below is not None:
            fraction = float(np.mean(peaks <= args.target_peak_below))
            attainment.append(
                {"target": f"peak <= {args.target_peak_below:g}", "fraction_attaining": fraction}
            )
        if args.target_auc is not None:
            fraction = float(np.mean(aucs >= args.target_auc))
            attainment.append(
                {"target": f"AUC >= {args.target_auc:g}", "fraction_attaining": fraction}
            )
        if args.target_trough is not None and args.target_peak_below is not None:
            both = float(
                np.mean((troughs >= args.target_trough) & (peaks <= args.target_peak_below))
            )
            attainment.append(
                {"target": "both trough and peak targets", "fraction_attaining": both}
            )
        if attainment:
