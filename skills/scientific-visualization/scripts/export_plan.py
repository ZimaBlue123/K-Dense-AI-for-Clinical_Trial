...
def build_plan(
    ...
    *,
    ...
    if width is not None and width not in widths:
        raise CliError(
            f"unknown width {width!r} for {publisher}; available: {', '.join(sorted(widths))}"
        )
...
def _effective_dimensions(
    ...
    if target_width_mm is not None and width_px:
        ...
        height_mm = float(height_px) / float(width_px) * target_width_mm if height_px else None
    ...
    dpi_values = [float(value) for value in (metadata.get("dpi_x"), metadata.get("dpi_y")) if value]
    return (
...
def validate_against_plan(
    ...
    *,
    ...
    effective_dpi, effective_width, effective_height = _effective_dimensions(metadata, target_width)
    ...
    if target_width is not None:
        if effective_width is None:
            ...
            width_status = (
                "pass" if abs(effective_width - target_width) <= width_tolerance_mm else "fail"
            )
        findings.append(
            _finding(
                "final_width_mm",
        ...
        findings.append(
            _finding(
                ...
                (
                    ...
                    else ("pass" if effective_height <= float(max_height) else "fail")
                ),
    ...
    if max_bytes is not None or max_bytes_exclusive is not None:
        ...
        if max_bytes_exclusive is not None:
            size_status = "pass" if actual_bytes < int(max_bytes_exclusive) else "fail"
        ...
        findings.append(
            _finding(
                ...
                (
                    ...
                    else ("pass" if normalized_mode in allowed_modes else "fail")
                ),
                actual=actual_mode,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--phase", help="your submission phase, for an explicit scope check")
