...
def export_figure(
    ...
    *,
    ...
    try:
        ...
        raise CliError(
            "Matplotlib is required for figure export; run with --with 'matplotlib==3.11.1'"
        ) from exc
    ...
    if overlap:
        raise CliError(
            "savefig_kwargs must not override explicit settings: " + ", ".join(sorted(overlap))
        )
            ...
            outputs.append(
                {
                    ...
                    "format": format_name,
                    "kind": ("vector-container" if format_name in VECTOR_FORMATS else "raster"),
...
def check_figure_size(
    ...
    *,
    ...
    try:
        ...
        raise CliError(
            f"unknown profile {journal!r}; available: {', '.join(sorted(document['profiles']))}"
        ) from exc
    ...
    candidates = {name: float(value) for name, value in (profile.get("widths_mm") or {}).items()}
    nearest_name = None
    nearest_width = None
    difference = None
    if candidates:
    ...
    return {
        ...
        "nearest_profile_width": {
            ...
            "matches_tolerance": (difference is not None and difference <= tolerance_mm),
        },
        "height_screen": {
            "maximum_mm": max_height,
            "within_snapshot": (None if max_height is None else height_mm <= float(max_height)),
        },
        "notice": (
            "A dimensional match to this dated snapshot is not a journal compliance determination."
        ),
    }
...
def save_for_journal(
    ...
    *,
    ...
    if dpi_profile.get("min_exclusive") is not None and dpi <= float(dpi_profile["min_exclusive"]):
        raise CliError(f"dpi does not exceed the profile snapshot threshold: {dpi_profile}")
...
def _demo_figure() -> Any:
    try:
        ...
        raise CliError(
            "Matplotlib is required for --demo; run with --with 'matplotlib==3.11.1'"
        ) from exc
