...
def build_preview(style_name: str, palette_name: str) -> tuple[Any, dict[str, Any]]:
    ...
    try:
        ...
        raise CliError(
            "Matplotlib is required for previews; run with --with 'matplotlib==3.11.1'"
        ) from exc
    ...
    with style_context(style_name, palette_name=palette_name):
        ...
        for index in range(4):
            values = [math.sin(value + index * 0.45) + index * 0.35 for value in x_values]
            axes[0, 0].plot(
                x_values,
                values,
        ...
        try:
            report = export_figure(
                ...
                provenance={
                    ...
                    "palette_screen": {
                        "background_review_count": audit["contrast_screen"]["review_count"],
                        "grayscale_review_count": audit["grayscale_screen"]["review_count"],
                    },
                },
