...
def audit_palette(
    ...
    *,
    ...
    if role not in ROLE_THRESHOLDS and contrast_threshold is None:
        raise CliError(f"unknown role {role!r}; provide an explicit contrast threshold")
    threshold = (
        float(contrast_threshold) if contrast_threshold is not None else ROLE_THRESHOLDS[role]
    )
        ...
        pair_records.append(
            {
                ...
                "grayscale_screen": ("pass" if delta >= grayscale_min_delta else "review"),
            }
        )

    background_reviews = sum(item["background_screen"] == "review" for item in color_records)
    grayscale_reviews = sum(item["grayscale_screen"] == "review" for item in pair_records)
    return {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    source.add_argument("--palette", help="bundled palette name (use --list-palettes)")
    source.add_argument(
        "--color",
        action="append",
        default=[],
    ...
    parser.add_argument(
        ...
        help=(f"heuristic minimum CIE L* separation (default: {DEFAULT_GRAYSCALE_DELTA})"),
    )
    parser.add_argument(
        "--fail-on",
    ...
    parser.add_argument("--force", action="store_true", help="overwrite an existing JSON report")
    return parser
    ...
    try:
        ...
        if args.palette:
            if args.palette not in palettes:
                raise CliError(
                    f"unknown palette {args.palette!r}; available: {', '.join(sorted(palettes))}"
                )
        ...
        should_fail = (args.fail_on in {"background", "any"} and background_review) or (
            args.fail_on in {"grayscale", "any"} and grayscale_review
        )
