...
def _load_palette_asset() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("_scientific_visualization_color_palettes", asset)
...
def get_style(
    ...
    *,
    ...
    if style_name not in STYLE_OVERRIDES:
        raise CliError(
            f"unknown style {style_name!r}; available: {', '.join(sorted(STYLE_OVERRIDES))}"
        )
    palettes = available_palettes()
    if palette_name not in palettes:
        raise CliError(
            f"unknown palette {palette_name!r}; available: {', '.join(sorted(palettes))}"
        )
...
def _matplotlib_style(style: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise CliError(
            "Matplotlib is required to apply styles; run with --with 'matplotlib==3.11.1'"
        ) from exc
    prepared = dict(style)
...
def style_context(
    ...
    *,
    ...
    try:
        ...
        raise CliError(
            "Matplotlib is required to use a style context; run with --with 'matplotlib==3.11.1'"
        ) from exc
    style = _matplotlib_style(get_style(style_name, palette_name=palette_name))
...
def set_color_palette(palette_name: str = "okabe_ito_on_white") -> list[str]:
    ...
    try:
        ...
        raise CliError(
            "Matplotlib is required to set a palette; run with --with 'matplotlib==3.11.1'"
        ) from exc
    palettes = available_palettes()
    if palette_name not in palettes:
        raise CliError(
...
def figure_size_for_profile(
    ...
    *,
    ...
    try:
        ...
        raise CliError(
            f"unknown publisher {publisher!r}; available: {', '.join(sorted(document['profiles']))}"
        ) from exc
    widths = profile.get("widths_mm") or {}
    if width not in widths:
        raise CliError(
            f"width {width!r} unavailable for {publisher}; available: {', '.join(sorted(widths))}"
        )
...
def configure_for_journal(
    ...
    *,
    ...
    profile_width = width_aliases.get(journal, {}).get(figure_width, figure_width)
    width_inches, height_inches = figure_size_for_profile(journal, profile_width, aspect=aspect)
    try:
        import matplotlib as mpl
    except ImportError as exc:
...
def create_style_template(
    ...
    *,
    ...
    lines.append("axes.prop_cycle: cycler('color', " + repr(colors).replace('"', "'") + ")")
    payload = ("\n".join(lines) + "\n").encode("utf-8")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--list", action="store_true", help="list available styles and palettes")
    parser.add_argument("--show", metavar="STYLE", help="print one style as JSON")
    parser.add_argument(
        "--write", nargs=2, metavar=("STYLE", "PATH"), help="write an mplstyle file"
    )
    parser.add_argument("--palette", default="okabe_ito_on_white", help="palette name")
    parser.add_argument("--force", action="store_true", help="overwrite an existing style file")
    return parser
                ...
                json.dumps(
                    {
                        ...
                        "palettes": {
                            name: colors for name, colors in sorted(available_palettes().items())
                        },
                    },
                    indent=2,
                    sort_keys=True,
