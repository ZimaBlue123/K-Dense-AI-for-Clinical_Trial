...
def inspect_raster(path: Path, *, max_pixels: int) -> dict[str, Any]:
    ...
    try:
        ...
        raise CliError(
            "Pillow is required for raster metadata; run with --with 'pillow==12.3.0'"
        ) from exc
        ...
        with warnings.catch_warnings():
            ...
            with Image.open(path) as image:
                ...
                if width * height > max_pixels:
                    raise CliError(f"image has {width * height} pixels; limit is {max_pixels}")
...
def inspect_svg(path: Path) -> dict[str, Any]:
    ...
    if path.stat().st_size > MAX_SVG_BYTES:
        raise CliError(f"SVG is {path.stat().st_size} bytes; limit is {MAX_SVG_BYTES} bytes")
    try:
        payload = path.read_bytes()
        if re.search(rb"<!ENTITY\b", payload, flags=re.IGNORECASE) or re.search(
            rb"<!DOCTYPE[^>]*\[", payload, flags=re.IGNORECASE | re.DOTALL
        ):
    ...
    for element in root.iter():
        ...
        if element_count > MAX_XML_ELEMENTS:
            raise CliError(f"SVG has more than {MAX_XML_ELEMENTS} elements; refusing input")
...
def _pdf_font_report(page: Any) -> dict[str, Any]:
    ...
    def embedded_status(font: Any) -> bool | None:
        try:
            ...
            if descendants:
                candidates.extend(reference.get_object() for reference in descendants)
    ...
    return {
        ...
        "all_embedded": (
            None if not definite or unknown_count else all(item is True for item in definite)
        ),
        "fonts": records,
...
def inspect_pdf(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise CliError(
            "pypdf is required for PDF metadata; run with --with 'pypdf==6.14.2'"
        ) from exc
    try:
...
def inspect_eps(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "height_mm": (height_pt / 72.0 * 25.4 if height_pt is not None else None),
        "dpi_x": None,
        "dpi_y": None,
        "mode": None,
...
def screen_metadata(
    ...
    *,
    ...
    for name, actual, minimum, maximum in (
        ...
        if actual is None:
            status = "unknown"
        elif minimum is not None and actual < minimum or maximum is not None and actual > maximum:
            status = "fail"
        else:
            status = "pass"
        checks.append(
