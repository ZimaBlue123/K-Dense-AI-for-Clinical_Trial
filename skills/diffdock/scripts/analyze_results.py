...
def print_summary(results, top_n=None, min_confidence=None):
    ...
    for complex_name, data in results.items():
        ...
        for pred in filtered_predictions:
            ...
            print(
                f"  Rank {pred['rank']:2d}: Confidence = {conf_str} ({confidence_class:8s}) | {pred['file']}"
            )
    ...
    for complex_name, data in results.items():
        ...
        for pred in predictions:
            if pred["confidence"] is not None:
                all_predictions.append({"complex": complex_name, **pred})
...
def print_top_predictions(results, n=10):
    ...
    for i, pred in enumerate(top_preds, 1):
        ...
        print(
            f"{i:2d}. {pred['complex']:30s} | Rank {pred['rank']:2d} | "
