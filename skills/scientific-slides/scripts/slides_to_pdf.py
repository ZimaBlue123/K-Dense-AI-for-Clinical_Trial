...
def combine_images_to_pdf(
    image_paths: list[Path], output_path: Path, dpi: int = 150, verbose: bool = False
) -> bool:
    """
    ...
    for i, img_path in enumerate(image_paths):
        try:
            ...
            if verbose:
                print(
                    f"  [{i + 1}/{len(image_paths)}] Loaded: {img_path.name} ({img.size[0]}x{img.size[1]})"
                )
    ...
    try:
        ...
        # Save to PDF
        first_image.save(
            output_path, "PDF", resolution=dpi, save_all=True, append_images=remaining_images
        )
...
Tips:
  ...
  - Standard presentation aspect ratio is 16:9 (1920x1080 or 1280x720)
    ...
    parser.add_argument("--dpi", type=int, default=150, help="PDF resolution in DPI (default: 150)")
    ...
    success = combine_images_to_pdf(image_files, output_path, dpi=args.dpi, verbose=args.verbose)
