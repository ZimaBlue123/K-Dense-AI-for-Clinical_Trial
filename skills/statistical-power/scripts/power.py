...
def _resolve(test, effect_size, alpha, alternative, **kw):
    ...
    if test == "t_ind":
        ...
        es = effect_size
        base = dict(
            effect_size=es, alpha=alpha, alternative=alternative, ratio=kw.get("ratio", 1.0)
        )
    ...
    if test == "two_proportions":
        ...
        base = dict(effect_size=h, alpha=alpha, alternative=alternative, ratio=kw.get("ratio", 1.0))
        return solver, base, "nobs1"
    ...
    if test == "correlation":
        ...
        return (
            "correlation",
            dict(effect_size=effect_size, alpha=alpha, alternative=alternative),
            "nobs",
        )
...
# --------------------------------------------------------------------------- #
def sample_size(
    test, effect_size=None, alpha=0.05, power=0.80, alternative="two-sided", round_up=True, **kw
):
    ...
    if solver == "regression":
        return _reg_sample_size(base["effect_size"], base["df_num"], base["k_total"], alpha, power)
...
def power(test, effect_size=None, nobs1=None, nobs=None, alpha=0.05, alternative="two-sided", **kw):
    ...
    if solver == "regression":
        return _reg_power(base["effect_size"], n, base["df_num"], base["k_total"], alpha)

    base[n_key] = n
    return float(solver.solve_power(**base))


def mde(test, nobs1=None, nobs=None, alpha=0.05, power=0.80, alternative="two-sided", **kw):
...
def power_curve(
    ...
    **kw,
    ...
    ax.plot(ns, pwr, lw=2)
    ax.axhline(
        power_target, ls="--", color="crimson", lw=1, label=f"target power = {power_target:g}"
    )
    ax.set_xlabel(
        "Sample size" + (" per group" if test in ("t_ind", "two_proportions") else " (total)")
    )
...
if __name__ == "__main__":
    ...
    print("t_ind  d=0.5, 80% power -> n/group =", sample_size("t_ind", effect_size=0.5, power=0.80))
    print(
        "anova  f=0.25, k=4, 80% -> total n =",
        sample_size("anova", effect_size=0.25, k_groups=4, power=0.80),
    )
    print(
        "2 props 0.40 vs 0.55, 80% -> n/group =",
        sample_size("two_proportions", prop1=0.40, prop2=0.55, power=0.80),
    )
    print(
        "correlation r=0.30, 80% -> n =", sample_size("correlation", effect_size=0.30, power=0.80)
    )
    print(
        "MDE for t_ind at n=30/group, 80% power -> d =",
        round(mde("t_ind", nobs1=30, power=0.80), 3),
    )
    print(
        "power for t_ind d=0.5 at n=64/group ->",
