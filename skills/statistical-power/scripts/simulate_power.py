...
class PowerEstimate:
    ...
    def __str__(self):
        return (
            f"n={self.n}: power={self.power:.3f} "
...
def simulate_power(gen_and_test, n, n_sims=2000, alpha=0.05, seed=0):
    ...
    return PowerEstimate(power=hits / n_sims, n_sims=n_sims, n=n, ci_low=lo, ci_high=hi)


def find_sample_size(
    gen_and_test, target_power=0.80, n_sims=2000, alpha=0.05, lo=10, hi=2000, seed=0, verbose=True
):
...
def example_cluster_randomized(effect=0.3, icc=0.05, cluster_size=20, resid_sd=1.0, alpha=0.05):
...
def example_linear_mixed_repeated(
    effect=0.4, n_timepoints=3, subj_sd=0.7, resid_sd=1.0, alpha=0.05
):
...
if __name__ == "__main__":
    ...
    n, est = find_sample_size(g, target_power=0.80, n_sims=500, lo=10, hi=60, verbose=False)
