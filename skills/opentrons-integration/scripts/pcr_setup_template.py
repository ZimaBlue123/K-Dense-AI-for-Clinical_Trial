...
def run(protocol: protocol_api.ProtocolContext):
    ...
    reagent_rack = protocol.load_labware(
        "opentrons_24_tuberack_nest_1.5ml_snapcap", "D1", label="Reagents"
    )
    ...
    master_mix = protocol.define_liquid(
        name="PCR Master Mix", description="2x PCR master mix", display_color="#FFB6C1"
    )

    template_dna = protocol.define_liquid(
        name="Template DNA", description="DNA samples", display_color="#90EE90"
    )
    ...
    tc_mod.set_block_temperature(
        temperature=95, hold_time_seconds=180, block_max_volume=total_reaction_volume
    )
    ...
    tc_mod.execute_profile(
        steps=profile, repetitions=num_cycles, block_max_volume=total_reaction_volume
    )

    # Final extension
    protocol.comment("Final extension...")
    tc_mod.set_block_temperature(
        temperature=72, hold_time_minutes=5, block_max_volume=total_reaction_volume
    )

    # Hold at 4°C
    protocol.comment("Cooling to 4°C for storage...")
    tc_mod.set_block_temperature(temperature=4, block_max_volume=total_reaction_volume)
