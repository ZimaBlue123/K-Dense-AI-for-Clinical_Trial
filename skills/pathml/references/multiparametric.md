...
# Load Vectra slide
vectra_slide = SlideData.from_slide("path/to/slide.qptiff", backend=SlideType.VectraQPTIFF)
...
vectra_pipeline = Pipeline(
    [
        ...
        # 2. Cell segmentation
        SegmentMIF(
            nuclear_channel="DAPI", cytoplasm_channel="FITC", model="mesmer", image_resolution=0.5
        ),
        # 3. Quantification
        QuantifyMIF(
            segmentation_mask_name="cell_segmentation",
...
# Apply annotation
thresholds = {"CD3": 0.5, "CD4": 0.4, "CD8": 0.4, "CD20": 0.3, "CD68": 0.3, "panCK": 0.5}
...
for i, cell_type in enumerate(cell_types):
    ...
    ax.scatter(coords[:, 0], coords[:, 1], c=[colors[i]], label=cell_type, s=5, alpha=0.7)
...
sq.pl.co_occurrence(
    adata, cluster_key="cell_type", clusters=["CD8 T cell", "Tumor"], figsize=(8, 8)
)
```
...
sq.gr.spatial_autocorr(adata, mode="moran", genes=["CD3", "CD8", "PD1", "PDL1", "panCK"])
...
merfish_pipeline = Pipeline(
    [
        ...
        DecodeMERFISH(
            codebook="path/to/codebook.csv", error_correction=True, distance_threshold=0.5
        ),
        # 2. Cell segmentation
        SegmentMIF(
            nuclear_channel="DAPI",
        ...
        AssignTranscripts(
            segmentation_mask_name="cell_segmentation", transcript_coords="decoded_spots"
        ),
    ]
)
...
qc_metrics = assess_segmentation_quality(
    segmentation_mask, image, metrics=["cell_count", "mean_cell_size", "size_distribution"]
)
...
dataset.run(codex_pipeline, distributed=True, client=client, scheduler="distributed")
...
def export_to_giotto(adata, output_dir):
    ...
    # Cell coordinates
    pd.DataFrame(adata.obsm["spatial"], columns=["x", "y"], index=adata.obs_names).to_csv(
        f"{output_dir}/spatial_locs.csv"
    )
