...
```python
ax.errorbar(
    x, y, yerr=error, fmt="o-", linewidth=2, capsize=5, capthick=2, label="With uncertainty"
)
```
...
```python
scatter = ax.scatter(x, y, s=sizes * 100, c=colors, cmap="viridis", alpha=0.6, edgecolors="black")
...
for category in categories:
    ...
    ax.scatter(data[mask]["x"], data[mask]["y"], label=category, s=50, alpha=0.7)
ax.legend()
```
...
```python
ax.barh(categories, values, color="coral", edgecolor="black", linewidth=1.5)
...
```python
ax.bar(categories, values, yerr=errors, capsize=5, color="steelblue", edgecolor="black")
```

### Bar Chart with Patterns
```python
bars1 = ax.bar(
    x - width / 2, values1, width, label="Group 1", color="white", edgecolor="black", hatch="//"
)
bars2 = ax.bar(
    x + width / 2, values2, width, label="Group 2", color="white", edgecolor="black", hatch="\\\\"
)
```
...
```python
ax.hist(data, bins=30, density=True, alpha=0.7, edgecolor="black", label="Empirical")

# Overlay theoretical distribution
from scipy.stats import norm

x = np.linspace(data.min(), data.max(), 100)
ax.plot(x, norm.pdf(x, data.mean(), data.std()), "r-", linewidth=2, label="Normal fit")
ax.legend()
```
...
```python
ax.boxplot(
    [data1, data2, data3], labels=["Group A", "Group B", "Group C"], showmeans=True, meanline=True
)
ax.set_ylabel("Values")
```
...
### Horizontal Box Plot
```python
ax.boxplot([data1, data2, data3], vert=False, labels=["Group A", "Group B", "Group C"])
ax.set_xlabel("Values")
```

### Violin Plot
```python
parts = ax.violinplot([data1, data2, data3], positions=[1, 2, 3], showmeans=True, showmedians=True)
ax.set_xticks([1, 2, 3])
...
contour = ax.contour(X, Y, Z, levels=10, colors="black", linewidths=0.5, alpha=0.4)
ax.clabel(contour, inline=True, fontsize=8)
plt.colorbar(contourf, ax=ax)
```
...
### Donut Chart
```python
ax.pie(sizes, labels=labels, autopct="%1.1f%%", wedgeprops=dict(width=0.5), startangle=90)
ax.axis("equal")
```
...
ax.fill_between(x, y1, y2, where=(y2 >= y1), alpha=0.3, label="y2 > y1", interpolate=True)
ax.legend()
```
...
polygon = Polygon(vertices, closed=True, edgecolor="black", facecolor="lightblue", alpha=0.5)
...
```python
ax.broken_barh([(10, 50), (100, 20), (130, 10)], (10, 9), facecolors="tab:blue")
ax.broken_barh([(10, 20), (50, 50), (120, 30)], (20, 9), facecolors="tab:orange")
ax.set_ylim(5, 35)
ax.set_xlim(0, 200)
ax.set_xlabel("Time")
ax.set_yticks([15, 25])
