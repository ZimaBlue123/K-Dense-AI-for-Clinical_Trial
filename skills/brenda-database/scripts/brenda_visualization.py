...
def plot_kinetic_parameters(ec_number: str, save_path: str = None, show_plot: bool = True) -> str:
    ...
    try:
        ...
        ax1.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Mean: {np.mean(km_values):.2f}",
        )
        ax1.axvline(
            np.median(km_values),
            color="blue",
            linestyle="--",
            label=f"Median: {np.median(km_values):.2f}",
        )
        ax1.legend()
...
def plot_organism_comparison(
    ec_number: str, organisms: list[str], save_path: str = None, show_plot: bool = True
) -> str:
...
def plot_pH_profiles(ec_number: str, save_path: str = None, show_plot: bool = True) -> str:
    ...
    try:
        ...
        ax2.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Mean: {np.mean(ph_values):.2f}",
        )
        ax2.axvline(
            np.median(ph_values),
            color="blue",
            linestyle="--",
            label=f"Median: {np.median(ph_values):.2f}",
        )
        ax2.legend()
...
def plot_temperature_profiles(ec_number: str, save_path: str = None, show_plot: bool = True) -> str:
    ...
    try:
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        fig.suptitle(
            f"Temperature Activity Profiles for EC {ec_number}",
            fontsize=16,
            fontweight="bold",
        )
        ...
        if len(temp_values) > 2:
            ...
            ax1.axvline(
                ...
                color="green",
                linestyle=":",
                label=f"Optimal: {optimum_temp:.1f}°C",
            )
            ax1.legend()
...
def plot_substrate_specificity(
    ...
    try:
        ...
        if PANDAS_AVAILABLE and kms:
            ...
            ax1.set_yticklabels(
                [s[:30] + "..." if len(s) > 30 else s for s in top_sorted["Substrate"]]
            )
            ax1.set_xlabel("Km (mM)")
            ...
            ax3.axvline(
                ...
                color="red",
                linestyle="--",
                label=f"Mean: {np.mean(kms):.2f}",
            )
            ax3.axvline(
                np.median(kms),
                color="blue",
                linestyle="--",
                label=f"Median: {np.median(kms):.2f}",
            )
            ax3.legend()
...
def plot_michaelis_menten(
    ec_number: str, substrate: str = None, save_path: str = None, show_plot: bool = True
) -> str:
    ...
    try:
        ...
        if not km:
            print("No Km data available for plotting")
            return save_path

        # Create figure
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
        fig.suptitle(
            f"Michaelis-Menten Kinetics for EC {ec_number}"
...
def plot_heatmap(ec_number: str, save_path: str = None, show_plot: bool = True) -> str:
    ...
    try:
        ...
        fig.suptitle(
            f"Enzyme Properties Heatmap for EC {ec_number}",
            fontsize=16,
            fontweight="bold",
        )

        # Plot 1: Raw data heatmap
        heatmap_data_raw = df.set_index("Organism")[
            ["Avg Km (mM)", "Optimal pH", "Optimal Temp (°C)", "Data Points"]
        ].T
        sns.heatmap(heatmap_data_raw, annot=True, fmt=".2f", cmap="viridis", ax=ax1)
        ax1.set_title("Raw Values")

        # Plot 2: Normalized data heatmap
        heatmap_data_norm = df_normalized.set_index("Organism")[
            ["Avg Km (mM)", "Optimal pH", "Optimal Temp (°C)", "Data Points"]
        ].T
...
def generate_summary_plots(ec_number: str, save_dir: str = None) -> list[str]:
    ...
    try:
        ...
        result_path = plot_organism_comparison(
            ec_number, model_organisms, save_path=save_path, show_plot=False
        )
        if result_path:
            generated_files.append(result_path)
