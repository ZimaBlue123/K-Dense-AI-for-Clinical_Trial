...
try:
    from brenda_client import call_brenda, get_km_values, get_reactions
                ...
                specificity.append(
                    {
                        ...
                        "vmax": (
                            sum(data["vmax_values"]) / len(data["vmax_values"])
                            if data["vmax_values"]
                            else None
                        ),
                        "kcat": (
                            sum(data["kcat_values"]) / len(data["kcat_values"])
                            if data["kcat_values"]
                            else None
                        ),
                        ...
                        activators.append(
                            {
                                ...
                                "type": (
                                    ...
                                    if "+" in activator
                                        ...
                                        "reducing agent"
                                        if "dtt" in activator.lower()
                                        or "mercapto" in activator.lower()
                                        else "other"
                                    )
                                ),
                                "mechanism": (
...
def find_ph_stable_variants(
    ec_number: str, min_ph: float = 8.0, max_ph: float = 6.0
) -> list[dict[str, Any]]:
...
def get_modeling_parameters(ec_number: str, substrate: str = None) -> dict[str, Any]:
    ...
    try:
        ...
        modeling_data = {
            ...
            "km": sum(kms) / len(kms) if kms else None,
            "km_std": (
                (sum((x - sum(kms) / len(kms)) ** 2 for x in kms) / len(kms)) ** 0.5
                if kms
                else None
            ),
            ...
            "data_points": len(km_data),
            "temperature": (
                sum(temperatures) / len(temperatures) if temperatures else 25.0
            ),  # Default to 25°C
