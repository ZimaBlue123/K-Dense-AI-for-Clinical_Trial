...
def run_sorting(
    ...
    print(
        f"Recording: {recording.get_num_channels()} channels, {recording.get_total_duration():.1f}s"
    )
    ...
    print(
        f"  Total spikes: {sum(len(sorting.get_unit_spike_train(uid)) for uid in sorting.unit_ids)}"
    )
