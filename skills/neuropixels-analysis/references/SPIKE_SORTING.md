...
# Run Kilosort4
sorting = si.run_sorter("kilosort4", recording, output_folder="ks4_output", verbose=True)
...
comparison = si.compare_multiple_sorters(
    [sorting_ks4, sorting_sc2, sorting_ms5], name_list=["KS4", "SC2", "MS5"]
)
...
recordings = [si.read_spikeglx(f"/path/to/recording_{i}", stream_id="imec0.ap") for i in range(3)]
...
for i, segment in enumerate(recording.split_by_times([0, 3600, 7200, 10800])):
    sorting_seg = si.run_sorter(
        "kilosort4", segment, output_folder=segments_output / f"segment_{i}"
    )
