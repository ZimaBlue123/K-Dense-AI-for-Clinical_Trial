...
# Run Bombcell
results = bc.run_bombcell(kilosort_folder, raw_data_path, sample_rate=30000, n_channels=384)
...
results = bc.run_bombcell(kilosort_folder, raw_data_path, **custom_params)
```
...
def curate_sorting(sorting, recording, output_dir):
    ...
    analyzer = si.create_sorting_analyzer(
        sorting, recording, sparse=True, folder=f"{output_dir}/analyzer"
    )
