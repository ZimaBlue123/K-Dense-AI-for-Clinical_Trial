...
```python
analyzer.compute(
    "quality_metrics", metric_names=["snr", "isi_violations_ratio", "presence_ratio", "firing_rate"]
)
```
...
### Export Report
```python
si.export_report(analyzer, output_folder="report", remove_if_exists=True, format="html")
```
